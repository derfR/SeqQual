...
if (-d "aln_replace3"){die "already exist dir aln_replace3\n";}
system ("mkdir aln_replace3");
...
foreach $input(@ids){
	...
	for ($i=0;$i<scalar(@seqs);$i++){
	    ...
	    splice (@seqs2,$i,1,"?");
 	    ...
 	    if ($seqs[$i-5] eq "?" &&  $seqs[$i-4] eq "?" &&  $seqs[$i-3] eq "?" &&  $seqs[$i-2] eq "?" &&  $seqs[$i-1] eq "?" &&  $seqs[$i+3] eq "?"  &&  $seqs[$i+4] eq "?" &&  $seqs[$i+5] eq "?" &&  $seqs[$i+6] eq "?" &&  $seqs[$i+7] eq "?"){
	    splice (@seqs2,$i,1,"?");
	    }

	    if ($seqs[$i-6] eq "?" &&  $seqs[$i-5] eq "?" &&  $seqs[$i-4] eq "?" &&  $seqs[$i-3] eq "?" &&  $seqs[$i-2] eq "?" &&  $seqs[$i+2] eq "?"  &&  $seqs[$i+3] eq "?" &&  $seqs[$i+4] eq "?" &&  $seqs[$i+5] eq "?" &&  $seqs[$i+6] eq "?"){
	    splice (@seqs2,$i,1,"?");
	    }

	    if ($seqs[$i-7] eq "?" &&  $seqs[$i-6] eq "?" &&  $seqs[$i-5] eq "?" &&  $seqs[$i-4] eq "?" &&  $seqs[$i-3] eq "?" &&  $seqs[$i+1] eq "?"  &&  $seqs[$i+2] eq "?" &&  $seqs[$i+3] eq "?" &&  $seqs[$i+4] eq "?" &&  $seqs[$i+5] eq "?"){
	    splice (@seqs2,$i,1,"?");
	    }
