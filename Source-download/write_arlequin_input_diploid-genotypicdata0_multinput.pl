...
if (-d "arlequin_input0"){die "already exist dir arlequin_input0\n";}
system ("mkdir arlequin_input0");
...
foreach $input(@ids){
    ...
    $input2=~s/\.aln//g;
    print $input2,"\n";
    $goal="$input2".".genotypicdata0.arp";
	...
	$size1=scalar(keys %entry1)*2;$size2=scalar(keys %entry2)*2;$size3=scalar(keys %entry3)*2;$size4=scalar(keys %entry4)*2;$size5=scalar(keys %entry5)*2;$size6=scalar(keys %entry6)*2;$size7=scalar(keys %entry7)*2;$size8=scalar(keys %entry8)*2;$size9=scalar(keys %entry9)*2;$size10=scalar(keys %entry10)*2;$size11=scalar(keys %entry11)*2;$size12=scalar(keys %entry12)*2;$size13=scalar(keys %entry13)*2;$size14=scalar(keys %entry14)*2;$size15=scalar(keys %entry15)*2;$size16=scalar(keys %entry16)*2;$size17=scalar(keys %entry17)*2;$size18=scalar(keys %entry18)*2;$size19=scalar(keys %entry19)*2;$size20=scalar(keys %entry20)*2;
	...
	if (%entry1){
		...
		$seqprint1=join(//,@seqs1); $seqprint2=join(//,@seqs2);
	        $ida=$id."-a";$idb=$id."-b";
		for ($i=0;$i<50-length($ida);$i++){$ida=$ida." ";}
	    	for ($i=0;$i<50-length($idb);$i++){$idb=$idb." ";}
		print OUT $ida,"\t1\t",$seqprint1,"\n";
		print OUT $idb,"\t1\t",$seqprint2,"\n";
	    }
	    print OUT "}\n";
	}
    ...
    system ("mv $goal arlequin_input0/"); 
}
